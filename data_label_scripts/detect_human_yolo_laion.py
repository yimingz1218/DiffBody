...
def main():
    ...
    os.makedirs("/fsx_laion/alvin/Dataset/laion2B-en-aesthetic-4.5plus-512_human", exist_ok=True)
    ...
    for tar_file in tqdm(all_tar_list):
        try:
            ...
            sink = wds.TarWriter(os.path.join("/fsx_laion/alvin/Dataset/laion2B-en-aesthetic-4.5plus-512_human", tar_file.split('/')[-1]))
            with torch.no_grad():
            ...
            os.system(f"rm -f {os.path.join('/fsx_laion/alvin/Dataset/laion2B-en-aesthetic-4.5plus-512_human', tar_file.split('/')[-1])}")
