...
def main():
    ...
    os.makedirs("/fsx_laion/alvin/Dataset/getty_human_newpose", exist_ok=True)
    ...
    for tar_file in tqdm(all_tar_list):
        ...
        os.system(f"rm -f {os.path.join('/fsx_laion/alvin/Dataset/getty_human_newpose', tar_name)}")
        writer = wds.TarWriter(os.path.join("/fsx_laion/alvin/Dataset/getty_human_newpose", tar_name))
        ...
        sample_count = 0
        for sample1, sample2 in zip(dataset, dataset_inpaint):
        ...
        writer.close()
        # except:
        #     os.system(f"rm -f {os.path.join('/fsx_laion/alvin/Dataset/getty_human_newpose', tar_file.split('/')[-1])}")
