...
def main():
    ...
    os.makedirs('/fsx_laion/alvin/Dataset/laion_human_depnorm', exist_ok=True)
    cnt = 0
    fail_list = []
    for tar_file in tqdm(all_tar_list):
        try:
            writer = wds.TarWriter(os.path.join("/fsx_laion/alvin/Dataset/laion_human_depnorm", tar_file.split('/')[-1]))
            ...
            os.system(f"rm -f {os.path.join('/fsx_laion/alvin/Dataset/laion_human_depnorm', tar_file.split('/')[-1])}")
