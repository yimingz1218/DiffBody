...
def main():
    ...
    os.makedirs('/fsx_laion/alvin/Dataset/getty_human_final', exist_ok=True)
    cnt = 0
    fail_list = []
    for tar_file in all_tar_list:
        # try:
        tar_name = tar_file.split('/')[-1]
        os.system(f"rm -f {os.path.join('/fsx_laion/alvin/Dataset/getty_human_final', tar_name)}")
        writer = wds.TarWriter(os.path.join("/fsx_laion/alvin/Dataset/getty_human_final", tar_name))
        dataset_ori = wds.WebDataset(tar_file)
        dataset_new = wds.WebDataset(os.path.join("/fsx_laion/alvin/Dataset/getty_human_newpose", tar_name))
        # sample_count = 0
        ...
        writer.close()
        # except:
        #     os.system(f"rm -f {os.path.join('/fsx_laion/alvin/Dataset/getty_human_final', tar_file.split('/')[-1])}")
