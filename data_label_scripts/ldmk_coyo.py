...
def main():
    ...
    os.makedirs('/fsx_laion/alvin/Dataset/coyo_human_ldmk', exist_ok=True)
    cnt = 0
    fail_list = []
    for tar_file in tqdm(all_tar_list):
        try:
            os.system(f"sudo rm -f {os.path.join('/fsx_laion/alvin/Dataset/coyo_human_ldmk', tar_file.split('/')[-1])}")
            writer = wds.TarWriter(os.path.join("/fsx_laion/alvin/Dataset/coyo_human_ldmk", tar_file.split('/')[-1]))
