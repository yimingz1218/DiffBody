...
def parse_args():
    ...
    parser.add_argument('--distill_cond_type', type=str, default=[], nargs="+", help='add which types of conditions')
...
def main():
    ...
    if args.distill_path is not None:
        ...
        unet_d = UNet2DConditionModel.from_pretrained(args.distill_path, subfolder="unet_ema")
        unet_d.requires_grad_(False)
    ...
    unet_d.to(accelerator.device, dtype=weight_dtype)
    ...
    if accelerator.is_main_process:
        ...
        tracker_config.pop("distill_cond_type")
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if "blip" in args.cond_type or "blip" in args.distill_cond_type:
                blip = blip.to(text_encoder.device)
                blip = blip.squeeze(dim=1)
                batch["blip"] = blip
            if "normal" in args.cond_type or "normal" in args.distill_cond_type:
                normal = normal.to(unet.device)
                batch["normal"] = normal
            if "depth" in args.cond_type or "depth" in args.distill_cond_type:
                depth = depth.to(unet.device)
                batch["depth"] = depth
            if "canny" in args.cond_type or "canny" in args.distill_cond_type:
                canny = canny.to(unet.device)
                batch["canny"] = canny
            if "body" in args.cond_type or "body" in args.distill_cond_type:
                body = body.to(unet.device)
                batch["body"] = body
            if "face" in args.cond_type or "face" in args.distill_cond_type:
                face = face.to(unet.device)
                batch["face"] = face
            if "hand" in args.cond_type or "hand" in args.distill_cond_type:
            ...
            with accelerator.accumulate(unet):
                ...
                if args.distill_path is not None:
                    if args.cond_inject == "concat":
                        noisy_latents_d = torch.cat([noisy_latents, depth_latents], dim=1) if "depth" in args.distill_cond_type else noisy_latents
                        noisy_latents_d = torch.cat([noisy_latents_d, normal_latents], dim=1) if "normal" in args.distill_cond_type else noisy_latents_d
                        noisy_latents_d = torch.cat([noisy_latents_d, canny_latents], dim=1) if "canny" in args.distill_cond_type else noisy_latents_d
                        noisy_latents_d = torch.cat([noisy_latents_d, body_latents], dim=1) if "body" in args.distill_cond_type else noisy_latents_d
                        noisy_latents_d = torch.cat([noisy_latents_d, face_latents], dim=1) if "face" in args.distill_cond_type else noisy_latents_d
                        noisy_latents_d = torch.cat([noisy_latents_d, hand_latents], dim=1) if "hand" in args.distill_cond_type else noisy_latents_d
                    elif args.cond_inject == "sum":
                        if len(args.distill_cond_type) == 0:
                            pass
                        else:
                            if args.cond_reshape == "vae":
                                channel_dim = 4
                                ...
                                channel_dim = args.embedder_channel
                            sum_latents = torch.zeros((noisy_latents.shape[0], channel_dim, h, w)).to(unet.device)
                            sum_latents = sum_latents + depth_latents if "depth" in args.distill_cond_type else sum_latents
                            sum_latents = sum_latents + normal_latents if "normal" in args.distill_cond_type else sum_latents
                            sum_latents = sum_latents + canny_latents if "canny" in args.distill_cond_type else sum_latents
                            sum_latents = sum_latents + body_latents if "body" in args.distill_cond_type else sum_latents
                            sum_latents = sum_latents + face_latents if "face" in args.distill_cond_type else sum_latents
                            sum_latents = sum_latents + hand_latents if "hand" in args.distill_cond_type else sum_latents
                            noisy_latents_d = torch.cat([noisy_latents, sum_latents], dim=1)
                ...
                if args.distill_path is not None:
                    with torch.no_grad():
                        target = unet_d(noisy_latents_d.detach(), timesteps.detach(), encoder_hidden_states.detach(), added_cond_kwargs=batch["unet_added_conditions"]).sample.detach()
