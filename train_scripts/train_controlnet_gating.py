...
def main(args):
    ...
    gating_unet.requires_grad_(True)
    ...
    controlnet, gating_unet, optimizer, train_dataloader, lr_scheduler = accelerator.prepare(
        controlnet, gating_unet, optimizer, train_dataloader, lr_scheduler
    )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(controlnet):
                ...
                midas_depth_latents = vae.encode(batch["midas_depth"].to(dtype=weight_dtype)).latent_dist.sample()
                ...
                normal_latents = vae.encode(batch["normal"].to(dtype=weight_dtype)).latent_dist.sample()
                ...
                whole_latents = vae.encode(batch["whole"].to(dtype=weight_dtype)).latent_dist.sample()
                ...
                input_to_gating = torch.cat([noisy_latents, midas_depth_latents, normal_latents, whole_latents], dim=1)
                gating_matrix = gating_unet(
                    input_to_gating,
                    timesteps,
                    encoder_hidden_states=encoder_hidden_states,
                ).sample
                # shape is (B, 3, H, W)
                gating_matrix = F.softmax(gating_matrix, dim=1)

                # controlnet_image = batch["conditioning_pixel_values"].to(dtype=weight_dtype)
                controlnet_image = []
                for key in ['depth', 'midas_depth', 'normal', 'canny', 'body', 'face', 'hand', 'whole']:
                    if key in args.cond_type:
                        controlnet_image.append(batch[key])
                ...
                if accelerator.sync_gradients:
                    params_to_clip = list(controlnet.parameters()) + list(gating_unet.parameters())
    ...
    if accelerator.is_main_process:
        ...
        gating_unet = accelerator.unwrap_model(gating_unet)
        gating_unet.save_pretrained(args.output_dir)
