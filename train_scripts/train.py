...
def log_validation(vae, text_encoder, tokenizer, unet, args, accelerator, weight_dtype, global_step, batch=None, depth_embedder=None, midas_depth_embedder=None, normal_embedder=None, canny_embedder=None, body_embedder=None, face_embedder=None, hand_embedder=None, whole_embedder=None):
    ...
    # else:
        ...
        if "midas_depth" in args.cond_type:
            # structural_cond.append(batch["midas_depth"])
            # save the condition images
            midas_depth_denormalize = (batch["midas_depth"] + 1) / 2.0
            midas_depth_numpy = midas_depth_denormalize.cpu().permute(0, 2, 3, 1).float().numpy()[0]
            midas_depth_numpy = (midas_depth_numpy * 255).round().astype("uint8")
            midas_depth_pil = Image.fromarray(midas_depth_numpy)
            midas_depth_pil.save(os.path.join(save_path, f"midas_depth.png"))
    ...
    for i in range(len(args.validation_prompts)):
        with torch.autocast("cuda"):
            ...
            output = pipeline(
                ...
                midas_depth_embedder=midas_depth_embedder, 
            ...
            if "midas_depth" in args.pred_cond:
                midas_depth_image = output.midas_depth_image[0]
                midas_depth_image.save(os.path.join(save_path, f"{i}-midas_depth.png"))
...
def parse_args():
    ...
    parser.add_argument('--target_change', type=str, choices=['depth', 'midas_depth', 'normal', 'canny', 'body', 'face', 'hand', 'whole'], help='how to inject the spatial condition')
    ...
    parser.add_argument("--midas_depth_weight", type=float, default=0.1)
...
def main():
    ...
    if args.cond_inject == "spade":
        num_cond = 0
        if "depth" in args.cond_type: num_cond += 1
        if "midas_depth" in args.cond_type: num_cond += 1
        if "normal" in args.cond_type: num_cond += 1
        if "canny" in args.cond_type: num_cond += 1
        if "body" in args.cond_type: num_cond += 1
        if "face" in args.cond_type: num_cond += 1
        ...
        if args.pred_cond is not None:
            num_cond = len(args.pred_cond)
            # if "depth" in args.cond_type: num_cond += 1
            # if "normal" in args.cond_type: num_cond += 1
            # if "canny" in args.cond_type: num_cond += 1
            # if "body" in args.cond_type: num_cond += 1
    ...
    depth_embedder = midas_depth_embedder = normal_embedder = canny_embedder = body_embedder = face_embedder = hand_embedder = whole_embedder = None
    ...
    if args.cond_reshape == "learn_conv":
        ...
        if "midas_depth" in args.cond_type:
            midas_depth_embedder = Embedder(conditioning_embedding_channels=args.embedder_channel, conditioning_channels=3).to(accelerator.device, dtype=weight_dtype)
            params_to_optimize.extend(list(midas_depth_embedder.parameters()))
        if "normal" in args.cond_type:
    ...
    if args.resume_from_checkpoint:
        ...
        if path is None:
            ...
            if args.cond_reshape == "learn_conv":
                ...
                if "midas_depth" in args.cond_type:
                    midas_depth_state_dict = torch.load(os.path.join(args.output_dir, path, "midas_depth_embedder.pth"))
                    new_state_dict = OrderedDict()
                    for k, v in midas_depth_state_dict.items():
                        name = k[7:] if k[:6] == 'module' else k 
                        new_state_dict[name] = v
                    midas_depth_embedder.load_state_dict(new_state_dict)
                if "normal" in args.cond_type:
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if args.target_change is not None:
                if args.target_change == 'depth':
                    images = depth
                if args.target_change == 'midas_depth':
                    images = midas_depth
            ...
            if "normal" in args.cond_type:
                ...
                batch["normal"] = normal
                if args.hierarchy_distill:
                    normal_dt = normal_dt.to(unet.device)
                    batch["normal"] = torch.cat([normal, normal_dt])
            if "depth" in args.cond_type:
                depth = depth.to(unet.device)
                batch["depth"] = depth
                if args.hierarchy_distill:
                    depth_dt = depth_dt.to(unet.device)
                    batch["depth"] = torch.cat([depth, depth_dt])
            if "midas_depth" in args.cond_type:
                midas_depth = midas_depth.to(unet.device)
                batch["midas_depth"] = midas_depth
                if args.hierarchy_distill:
                    midas_depth_dt = midas_depth_dt.to(unet.device)
                    batch["midas_depth"] = torch.cat([midas_depth, midas_depth_dt])
            if "canny" in args.cond_type:
                canny = canny.to(unet.device)
                batch["canny"] = canny
                if args.hierarchy_distill:
                    canny_dt = canny_dt.to(unet.device)
                    batch["canny"] = torch.cat([canny, canny_dt])
            if "body" in args.cond_type:
                body = body.to(unet.device)
                batch["body"] = body
                if args.hierarchy_distill:
                    body_dt = body_dt.to(unet.device)
                    batch["body"] = torch.cat([body, body_dt])
            if "face" in args.cond_type:
                face = face.to(unet.device)
                batch["face"] = face
                if args.hierarchy_distill:
                    face_dt = face_dt.to(unet.device)
                    batch["face"] = torch.cat([face, face_dt])
            if "hand" in args.cond_type:
                hand = hand.to(unet.device)
                batch["hand"] = hand
                if args.hierarchy_distill:
                    hand_dt = hand_dt.to(unet.device)
                    batch["hand"] = torch.cat([hand, hand_dt])
            if "whole" in args.cond_type:
                whole = whole.to(unet.device)
                batch["whole"] = whole
                if args.hierarchy_distill:
                    whole_dt = whole_dt.to(unet.device)
                    batch["whole"] = torch.cat([whole, whole_dt])
            ...
            with accelerator.accumulate(unet):
                ...
                if "midas_depth" in args.cond_type:
                    if args.cond_reshape == "vae":
                        midas_depth_latents = vae.encode(batch["midas_depth"].to(weight_dtype)).latent_dist.sample()
                        midas_depth_latents = midas_depth_latents * vae.config.scaling_factor
                    elif args.cond_reshape == "resize":
                        midas_depth_latents = F.interpolate(batch['midas_depth'], (h,w))
                    elif args.cond_reshape == "learn_conv":
                        midas_depth_latents = midas_depth_embedder(batch['midas_depth'])
                    else:
                        assert False, "unknown condition reshape type"
                ...
                if "midas_depth" in args.cond_type:
                    ori_midas_depth_latents = midas_depth_latents
                    if "midas_depth" in args.noisy_cond:
                        noise_midas_depth = torch.randn_like(midas_depth_latents)
                        midas_depth_latents = noise_scheduler.add_noise(midas_depth_latents, noise_midas_depth, timesteps)
                ...
                if args.cond_inject == "concat":
                    ...
                    noisy_latents = torch.cat([noisy_latents, midas_depth_latents], dim=1) if "midas_depth" in args.cond_type else noisy_latents
                    ...
                    if len(args.cond_type) == 0:
                        ...
                        sum_latents = sum_latents + midas_depth_latents if "midas_depth" in args.cond_type else sum_latents
                ...
                if noise_scheduler.config.prediction_type == "epsilon":
                    ...
                    if "midas_depth" in args.cond_type and "midas_depth" in args.pred_cond:
                        if "midas_depth" in args.noisy_cond:
                            target_midas_depth = noise_midas_depth
                        else:
                            target_midas_depth = ori_midas_depth_latents
                    ...
                    if "midas_depth" in args.cond_type and "midas_depth" in args.pred_cond:
                        if "midas_depth" in args.noisy_cond:
                            target_midas_depth = noise_scheduler.get_velocity(ori_midas_depth_latents, noise, timesteps)
                        else:
                            target_midas_depth = ori_midas_depth_latents
                    if "normal" in args.cond_type and "normal" in args.pred_cond:
                        if "normal" in args.noisy_cond:
                    ...
                    if "whole" in args.cond_type and "whole" in args.pred_cond:
                        if "whole" in args.noisy_cond:
                            target_whole = noise_scheduler.get_velocity(ori_whole_latents, noise, timesteps)
                        else:
                            target_whole = ori_whole_latents
                else:
                ...
                if args.hierarchy_distill:
                    ori_eps, add_eps = model_pred.chunk(2)
                    distill_loss = F.mse_loss(ori_eps.float(), add_eps.float(), reduction="mean") * args.distill_weight
                    loss += distill_loss
                ...
                if "depth" in args.cond_type and "depth" in args.pred_cond:
                    ...
                    loss += depth_loss
                if "midas_depth" in args.cond_type and "midas_depth" in args.pred_cond:
                    midas_depth_loss = F.mse_loss(midas_depth_pred.float(), target_midas_depth.float(), reduction="mean") * args.midas_depth_weight
                    loss += midas_depth_loss
                if "normal" in args.cond_type and "normal" in args.pred_cond:
                    ...
                    if accelerator.is_main_process:
                        ...
                        if not os.path.exists(save_path):
                            ...
                            if args.cond_reshape == "learn_conv":
                                ...
                                if "midas_depth" in args.cond_type:
                                    torch.save(midas_depth_embedder.state_dict(), os.path.join(save_path, "midas_depth_embedder.pth"))
                                if "normal" in args.cond_type:
            ...
            if args.hierarchy_distill:
                logs["distill"] = distill_loss.detach().item()
            if "depth" in args.cond_type and "depth" in args.pred_cond:
                logs["depth"] = depth_loss.detach().item()
            if "midas_depth" in args.cond_type and "midas_depth" in args.pred_cond:
                logs["midas_depth"] = midas_depth_loss.detach().item()
            if "normal" in args.cond_type and "normal" in args.pred_cond:
