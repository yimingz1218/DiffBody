...
from pipelines.pipeline_stable_diffusion_spade_timemoe4 import StableDiffusionPipeline
...
def log_validation(vae, text_encoder, tokenizer, unet, unet2, unet3, unet4, args, accelerator, weight_dtype, global_step, batch=None, depth_embedder=None, normal_embedder=None, canny_embedder=None, body_embedder=None, face_embedder=None, hand_embedder=None):
...
def main():
    ...
    unet, unet2, unet3, unet4, optimizer, optimizer2, optimizer3, optimizer4, train_dataloader, lr_scheduler, lr_scheduler2, lr_scheduler3, lr_scheduler4 = accelerator.prepare(
        unet, unet2, unet3, unet4, optimizer, optimizer2, optimizer3, optimizer4, train_dataloader, lr_scheduler, lr_scheduler2, lr_scheduler3, lr_scheduler4
    )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet3):                
                ...
                bsz = latents.shape[0]
                # Sample a random timestep for each image
                timesteps = torch.randint(noise_scheduler.config.num_train_timesteps // 4 * 2, noise_scheduler.config.num_train_timesteps // 4 * 3, (bsz,), device=latents.device)
                timesteps = timesteps.long()
