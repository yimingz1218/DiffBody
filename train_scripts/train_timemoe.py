...
from pipelines.pipeline_stable_diffusion_spade_timemoe import StableDiffusionPipeline
...
def log_validation(vae, text_encoder, tokenizer, unet, unet2, args, accelerator, weight_dtype, global_step, batch=None, depth_embedder=None, normal_embedder=None, canny_embedder=None, body_embedder=None, face_embedder=None, hand_embedder=None):
...
def main():
    ...
    unet, unet2, optimizer, optimizer2, train_dataloader, lr_scheduler, lr_scheduler2 = accelerator.prepare(
        unet, unet2, optimizer, optimizer2, train_dataloader, lr_scheduler, lr_scheduler2
    )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):                
                ...
                timesteps = torch.randint(0, noise_scheduler.config.num_train_timesteps // 2, (bsz,), device=latents.device)
            ...
            with accelerator.accumulate(unet2):                
                ...
                bsz = latents.shape[0]
                # Sample a random timestep for each image
                timesteps = torch.randint(noise_scheduler.config.num_train_timesteps // 2, noise_scheduler.config.num_train_timesteps, (bsz,), device=latents.device)
                timesteps = timesteps.long()
            ...
            logs = {"step_loss": loss.detach().item(), "lr": lr_scheduler.get_last_lr()[0], "lr2": lr_scheduler2.get_last_lr()[0], "dm_loss": dm_loss.detach().item()}
            if "depth" in args.cond_type and "depth" in args.noisy_cond:
