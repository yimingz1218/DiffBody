...
def log_validation(vae, unet, controlnet, args, accelerator, weight_dtype, step, test_dataloader):
...
def parse_args(input_args=None):
    ...
    parser.add_argument('--bliphuman', default=False, action="store_true")
    parser.add_argument('--overfit', default=False, action="store_true")
    parser.add_argument("--dropout_text", type=float, default=0.0)
...
def main(args):
    ...
    def compute_embeddings(prompt, text_encoders, tokenizers, ori_h, ori_w, crops_coords_top_left_h, crops_coords_top_left_w):
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            ...
            add_text_embeds = []
            add_time_ids = []
            for i_sample, text in enumerate(text_raw):
                instance_prompt_hidden_states, instance_unet_added_conditions = compute_embeddings(
                    text, text_encoders, tokenizers, 
                    descriptions[i_sample]["h"], descriptions[i_sample]["w"], 
                    descriptions[i_sample]["crop_tl_h"], descriptions[i_sample]["crop_tl_w"],
                )
                text_input_ids.append(instance_prompt_hidden_states)
                add_text_embeds.append(instance_unet_added_conditions["text_embeds"])
                add_time_ids.append(instance_unet_added_conditions["time_ids"])
            text_input_ids = torch.cat(text_input_ids, dim=0).to(text_encoders[0].device)
            add_text_embeds = torch.cat(add_text_embeds, dim=0).to(text_encoders[0].device)
            add_time_ids = torch.cat(add_time_ids, dim=0).to(text_encoders[0].device)
            if args.hierarchy_distill:
                add_text_embeds = torch.cat([add_text_embeds] * 2)
                add_time_ids = torch.cat([add_time_ids] * 2)
            batch = {
                'pixel_values': images.to(weight_dtype),
                'input_ids': text_input_ids.to(weight_dtype),
                "unet_added_conditions": {"text_embeds": add_text_embeds.to(weight_dtype), "time_ids": add_time_ids.to(weight_dtype)},
            }
            
            if "blip" in args.cond_type:
                blip = blip.to(text_encoders[0].device)
                blip = blip.squeeze(dim=1)
                batch["blip"] = blip
            if "normal" in args.cond_type:
                normal = normal.to(unet.device)
