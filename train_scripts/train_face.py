...
from transformers import CLIPTextModel, CLIPFeatureExtractor, CLIPTokenizer, CLIPProcessor, CLIPVisionModel
...
from openclip.training.data import get_wds_dataset, get_wds_dataset_cond, get_wds_dataset_cond_face
...
def log_validation(vae, text_encoder, tokenizer, unet, args, accelerator, weight_dtype, global_step, batch=None, depth_embedder=None, normal_embedder=None, canny_embedder=None, body_embedder=None, face_embedder=None, hand_embedder=None):
    ...
    # else:
        ...
        if "ldmk" in args.cond_type:
            # structural_cond.append(batch["hand"])
            hand_denormalize = (batch["ldmk"] + 1) / 2.0
            hand_numpy = hand_denormalize.cpu().permute(0, 2, 3, 1).float().numpy()[0]
            hand_numpy = (hand_numpy * 255).round().astype("uint8")
            hand_pil = Image.fromarray(hand_numpy)
            hand_pil.save(os.path.join(save_path, f"ldmk.png"))
            batch["ldmk"] = batch["ldmk"][0].unsqueeze(0)
...
def main():
    ...
    data_args.seed = -1
    train_dataset = get_wds_dataset_cond_face(data_args,
                                  main_args=args,
                                  is_train=True,
                                  epoch=0,
                                  floor=False,
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            try:
                ...
                images, text_input_ids, ldmk = batch
                # print(images.shape, text_input_ids.shape, ldmk.shape)
                # # Normalize the tensor back to the range (0, 1)
                # normalized_tensor = (images[0] + 1) / 2
                # numpy_array = normalized_tensor.numpy()
                # pil_image = Image.fromarray((numpy_array * 255).astype('uint8').transpose(1, 2, 0))
                # pil_image.save(f"/fsx_laion/alvin/visualization/face/image-{step}.png")
                # # Normalize the tensor back to the range (0, 1)
                # normalized_tensor = (ldmk[0] + 1) / 2
                # numpy_array = normalized_tensor.numpy()
                # pil_image = Image.fromarray((numpy_array * 255).astype('uint8').transpose(1, 2, 0))
                # pil_image.save(f"/fsx_laion/alvin/visualization/face/image-{step}-ldmk.png")
                # if step >= 64:
                #     exit(0)
                # continue
                # images, text_input_ids, blip, normal, depth, canny = batch
                images = images.to(unet.device)
                ldmk = ldmk.to(unet.device)
                ...
                with accelerator.accumulate(unet):
                    ...
                    if "ldmk" in args.cond_type:
                        if args.cond_reshape == "vae":
                            ldmk_latents = vae.encode(batch["ldmk"].to(weight_dtype)).latent_dist.sample()
                            ldmk_latents = ldmk_latents * vae.config.scaling_factor
                        elif args.cond_reshape == "resize":
                    ...
                    if args.cond_inject == "concat":
                        ...
                        noisy_latents = torch.cat([noisy_latents, ldmk_latents], dim=1) if "ldmk" in args.cond_type else noisy_latents
                    elif args.cond_inject == "sum":
                        if args.cond_reshape == "vae":
                            channel_dim = 4
                        elif args.cond_reshape == "resize":
                        ...
                        sum_latents = sum_latents + ldmk_latents if "ldmk" in args.cond_type else sum_latents
                    ...
                    if args.cond_inject == "spade":
                        ...
                        structural_cond = torch.cat(structural_cond, dim=1)
                        model_pred = unet(noisy_latents, structural_cond, timesteps, encoder_hidden_states).sample
                    else:
