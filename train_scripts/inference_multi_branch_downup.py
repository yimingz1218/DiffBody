...
def log_validation(vae, text_encoder, tokenizer, unet, args, accelerator, weight_dtype, global_step, batch=None):
    ...
    if batch is not None:
        image_denormalize = (batch["image"] + 1) / 2.0
    ...
    for i_test, batch in enumerate(test_dataloader):
        ...
        for i_batch in range(batch_size):
            ...
            with torch.autocast("cuda"):
                ...
                midas_depth_image = output.midas_depth_image[0]
                midas_depth_image.save(os.path.join(save_path, f"denoise-midas_depth-{img_id}.png"))
                normal_image = output.normal_image[0]
                normal_image.save(os.path.join(save_path, f"denoise-normal-{img_id}.png"))
