...
from pipelines.pipeline_stable_diffusion_spade_timemoe3 import StableDiffusionPipeline
...
def log_validation(vae, text_encoder, tokenizer, unet, unet2, unet3, args, accelerator, weight_dtype, global_step, batch=None, depth_embedder=None, normal_embedder=None, canny_embedder=None, body_embedder=None, face_embedder=None, hand_embedder=None):
    ...
    pipeline = StableDiffusionPipeline.from_pretrained(
        ...
        unet2=accelerator.unwrap_model(unet2),
        unet3=accelerator.unwrap_model(unet3),
        safety_checker=None,
...
def main():
    ...
    unet2 = copy.deepcopy(unet)
    unet3 = copy.deepcopy(unet)
    # unet4 = copy.deepcopy(unet)
    unet.requires_grad_(True)
    unet2.requires_grad_(True)
    unet3.requires_grad_(True)
    # unet4.requires_grad_(True)
    ...
    if args.use_ema:
        if args.cond_inject == "spade":
            ...
            ema_unet2 = copy.deepcopy(ema_unet)
            ema_unet3 = copy.deepcopy(ema_unet)
            # ema_unet4 = copy.deepcopy(ema_unet)
            ema_unet = EMAModel(ema_unet.parameters(), model_cls=UNet2DConditionModel, model_config=ema_unet.config)
            ema_unet2 = EMAModel(ema_unet2.parameters(), model_cls=UNet2DConditionModel, model_config=ema_unet2.config)
            ema_unet3 = EMAModel(ema_unet3.parameters(), model_cls=UNet2DConditionModel, model_config=ema_unet3.config)
            # ema_unet4 = EMAModel(ema_unet4.parameters(), model_cls=UNet2DConditionModel, model_config=ema_unet4.config)
    # print(unet2)
    # print(ema_unet2)
    # exit(0)
    if args.enable_xformers_memory_efficient_attention:
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def save_model_hook(models, weights, output_dir):
            if args.use_ema:
                ...
                ema_unet2.save_pretrained(os.path.join(output_dir, "unet_ema2"))
                ema_unet3.save_pretrained(os.path.join(output_dir, "unet_ema3"))
                # ema_unet4.save_pretrained(os.path.join(output_dir, "unet_ema4"))
                
            # unet.save_pretrained(os.path.join(output_dir, "unet"))
            # unet2.save_pretrained(os.path.join(output_dir, "unet2"))
        ...
        def load_model_hook(models, input_dir):
            if args.use_ema:
                ...
                load_model = EMAModel.from_pretrained(os.path.join(input_dir, "unet_ema2"), UNet2DConditionModel)
                ema_unet2.load_state_dict(load_model.state_dict())
                ema_unet2.to(accelerator.device)
                load_model = EMAModel.from_pretrained(os.path.join(input_dir, "unet_ema3"), UNet2DConditionModel)
                ema_unet3.load_state_dict(load_model.state_dict())
                ema_unet3.to(accelerator.device)
                # load_model = EMAModel.from_pretrained(os.path.join(input_dir, "unet_ema4"), UNet2DConditionModel)
                # ema_unet4.load_state_dict(load_model.state_dict())
                # ema_unet4.to(accelerator.device)
                del load_model
                
            # load_model = UNet2DConditionModel.from_pretrained(input_dir, subfolder=f"unet")
            # unet.register_to_config(**load_model.config)
            # unet.load_state_dict(load_model.state_dict())
            
            # load_model = UNet2DConditionModel.from_pretrained(input_dir, subfolder=f"unet2")
            ...
            for i in range(len(models)):
                ...
                load_model = UNet2DConditionModel.from_pretrained(input_dir, subfolder=f"unet_{i}")
    ...
    if args.gradient_checkpointing:
        ...
        unet3.enable_gradient_checkpointing()
        # unet4.enable_gradient_checkpointing()
    ...
    params_to_optimize2 = [param for param in unet2.parameters() if param.requires_grad]
    params_to_optimize3 = [param for param in unet3.parameters() if param.requires_grad]
    # params_to_optimize4 = [param for param in unet4.parameters() if param.requires_grad]
    ...
    unet, unet2, unet3, optimizer, optimizer2, optimizer3, train_dataloader, lr_scheduler, lr_scheduler2, lr_scheduler3 = accelerator.prepare(
        unet, unet2, unet3, optimizer, optimizer2, optimizer3, train_dataloader, lr_scheduler, lr_scheduler2, lr_scheduler3
    )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            images, text_input_ids, text_raw, blip, blip_raw, body, face, hand, normal, depth, canny, descriptions, body_ori, face_ori, hand_ori, normal_ori, depth_ori, canny_ori = batch
            ...
            with accelerator.accumulate(unet):                
                ...
                timesteps = torch.randint(0, noise_scheduler.config.num_train_timesteps // 4, (bsz,), device=latents.device)
            ...
            with accelerator.accumulate(unet2):                
                # Sample noise that we'll add to the latents
                noise = torch.randn_like(latents)
                if args.noise_offset:
                    # https://www.crosslabs.org//blog/diffusion-with-offset-noise
                ...
                bsz = latents.shape[0]
                # Sample a random timestep for each image
                timesteps = torch.randint(noise_scheduler.config.num_train_timesteps // 4, noise_scheduler.config.num_train_timesteps // 4 * 2, (bsz,), device=latents.device)
                timesteps = timesteps.long()
                ...
                if args.cond_inject == "concat":
                    ...
                    if len(args.cond_type) == 0:
                        ...
                        if args.cond_reshape == "vae":
                            ...
                            channel_dim = args.embedder_channel
                        sum_latents = torch.zeros((noisy_latents.shape[0], channel_dim, h, w)).to(unet2.device)
                        sum_latents = sum_latents + depth_latents if "depth" in args.cond_type else sum_latents
                        sum_latents = sum_latents + normal_latents if "normal" in args.cond_type else sum_latents
                        sum_latents = sum_latents + canny_latents if "canny" in args.cond_type else sum_latents
                        sum_latents = sum_latents + body_latents if "body" in args.cond_type else sum_latents
                ...
                if args.cond_inject == "spade":
                    ...
                    if "hand" in args.cond_type:
                        structural_cond.append(batch["hand"])
                    structural_cond = torch.cat(structural_cond, dim=1)
                    model_pred = unet2(noisy_latents, structural_cond, timesteps, encoder_hidden_states, added_cond_kwargs=batch["unet_added_conditions"]).sample
                else:
                    model_pred = unet2(noisy_latents, timesteps, encoder_hidden_states, added_cond_kwargs=batch["unet_added_conditions"]).sample
                ...
                accelerator.backward(loss)
                if accelerator.sync_gradients:
                    accelerator.clip_grad_norm_(params_to_optimize2, args.max_grad_norm)
                optimizer2.step()
                lr_scheduler2.step()
                optimizer2.zero_grad()
                
            with accelerator.accumulate(unet3):                
                # Sample noise that we'll add to the latents
                noise = torch.randn_like(latents)
                if args.noise_offset:
                    # https://www.crosslabs.org//blog/diffusion-with-offset-noise
                ...
                bsz = latents.shape[0]
                # Sample a random timestep for each image
                timesteps = torch.randint(noise_scheduler.config.num_train_timesteps // 4 * 2, noise_scheduler.config.num_train_timesteps, (bsz,), device=latents.device)
                timesteps = timesteps.long()
                ...
                if args.cond_inject == "concat":
                    ...
                    if len(args.cond_type) == 0:
                        ...
                        if args.cond_reshape == "vae":
                            ...
                            channel_dim = args.embedder_channel
                        sum_latents = torch.zeros((noisy_latents.shape[0], channel_dim, h, w)).to(unet3.device)
                        sum_latents = sum_latents + depth_latents if "depth" in args.cond_type else sum_latents
                        sum_latents = sum_latents + normal_latents if "normal" in args.cond_type else sum_latents
                        sum_latents = sum_latents + canny_latents if "canny" in args.cond_type else sum_latents
                        sum_latents = sum_latents + body_latents if "body" in args.cond_type else sum_latents
                ...
                if args.cond_inject == "spade":
                    ...
                    if "hand" in args.cond_type:
                        structural_cond.append(batch["hand"])
                    structural_cond = torch.cat(structural_cond, dim=1)
                    model_pred = unet3(noisy_latents, structural_cond, timesteps, encoder_hidden_states, added_cond_kwargs=batch["unet_added_conditions"]).sample
                else:
                    model_pred = unet3(noisy_latents, timesteps, encoder_hidden_states, added_cond_kwargs=batch["unet_added_conditions"]).sample
                ...
                accelerator.backward(loss)
                if accelerator.sync_gradients:
                    accelerator.clip_grad_norm_(params_to_optimize3, args.max_grad_norm)
                optimizer3.step()
                lr_scheduler3.step()
                optimizer3.zero_grad()
                
            # with accelerator.accumulate(unet4):                
            #     # Sample noise that we'll add to the latents
            #     noise = torch.randn_like(latents)
            #     if args.noise_offset:
            #         # https://www.crosslabs.org//blog/diffusion-with-offset-noise
            ...
            #     bsz = latents.shape[0]
            #     # Sample a random timestep for each image
            #     timesteps = torch.randint(noise_scheduler.config.num_train_timesteps // 4 * 3, noise_scheduler.config.num_train_timesteps, (bsz,), device=latents.device)
            #     timesteps = timesteps.long()
            ...
            #             elif args.cond_reshape == "learn_conv":
            #                 channel_dim = args.embedder_channel
            #             sum_latents = torch.zeros((noisy_latents.shape[0], channel_dim, h, w)).to(unet4.device)
            #             sum_latents = sum_latents + depth_latents if "depth" in args.cond_type else sum_latents
            #             sum_latents = sum_latents + normal_latents if "normal" in args.cond_type else sum_latents
            #             sum_latents = sum_latents + canny_latents if "canny" in args.cond_type else sum_latents
            #             sum_latents = sum_latents + body_latents if "body" in args.cond_type else sum_latents
            ...
            #             structural_cond.append(batch["hand"])
            #         structural_cond = torch.cat(structural_cond, dim=1)
            #         model_pred = unet4(noisy_latents, structural_cond, timesteps, encoder_hidden_states, added_cond_kwargs=batch["unet_added_conditions"]).sample
            #     else:
            #         model_pred = unet4(noisy_latents, timesteps, encoder_hidden_states, added_cond_kwargs=batch["unet_added_conditions"]).sample
            ...
            #     accelerator.backward(loss)
            #     if accelerator.sync_gradients:
            #         accelerator.clip_grad_norm_(params_to_optimize4, args.max_grad_norm)
            #     optimizer4.step()
            #     lr_scheduler4.step()
            ...
            if accelerator.sync_gradients:
                if args.use_ema:
                    ...
                    ema_unet2.step(unet2.parameters())
                    ema_unet3.step(unet3.parameters())
                    # ema_unet4.step(unet4.parameters())
                progress_bar.update(1)
                global_step += 1
            ...
            if accelerator.is_main_process:
                ...
                if args.validation_prompts is not None and global_step % args.validation_steps == 0:
                    if args.use_ema:
                        ...
                        ema_unet3.store(unet3.parameters())
                        ema_unet3.copy_to(unet3.parameters())
                        # ema_unet4.store(unet4.parameters())
                        # ema_unet4.copy_to(unet4.parameters())
                    ...
                    if args.use_ema:
                        ...
                        ema_unet2.restore(unet2.parameters())
                        ema_unet3.restore(unet3.parameters())
                        # ema_unet4.restore(unet4.parameters())
    ...
    if accelerator.is_main_process:
        ...
        unet2 = accelerator.unwrap_model(unet2)
        unet3 = accelerator.unwrap_model(unet3)
        # unet4 = accelerator.unwrap_model(unet4)
        if args.use_ema:
            ema_unet.copy_to(unet.parameters())
            ema_unet2.copy_to(unet2.parameters())
            ema_unet3.copy_to(unet3.parameters())
