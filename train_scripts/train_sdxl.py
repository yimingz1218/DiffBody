...
def main():
    ...
    with ContextManagers(deepspeed_zero_init_disabled_context_manager()):
        ...
        vae = AutoencoderKL.from_pretrained(
            args.pretrained_model_name_or_path, subfolder="vae", revision=args.revision, variant="fp16" if args.mixed_precision=="fp16" else None
        )
    ...
    if args.cond_inject == "spade":
        ...
        unet = UNet2DConditionModel.from_pretrained_2d(args.pretrained_model_name_or_path, subfolder="unet", fp16=True if args.mixed_precision=="fp16" else False)
    ...
    if args.use_ema:
        if args.cond_inject == "spade":
            ...
            from diffusers import UNet2DConditionModel
            ema_unet = UNet2DConditionModel.from_pretrained_2d(args.pretrained_model_name_or_path, subfolder="unet", fp16=True if args.mixed_precision=="fp16" else False)
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            for i_sample, text in enumerate(text_raw):
                instance_prompt_hidden_states, instance_unet_added_conditions = compute_embeddings(
                    ...
                    description[i_sample]["h"], description[i_sample]["w"], 
                    description[i_sample]["crop_tl_h"], description[i_sample]["crop_tl_w"],
                )
            ...
            with accelerator.accumulate(unet):
                ...
                if args.cond_inject == "spade":
                    ...
                    if "hand" in args.cond_type:
                        structural_cond.append(batch["hand"])
                    structural_cond = torch.cat(structural_cond, dim=1)
                    model_pred = unet(noisy_latents, structural_cond, timesteps, batch["input_ids"], added_cond_kwargs=batch["unet_added_conditions"]).sample
                else:
                    model_pred = unet(noisy_latents, timesteps, batch["input_ids"], added_cond_kwargs=batch["unet_added_conditions"]).sample
