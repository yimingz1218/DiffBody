...
def main():
    ...
    os.makedirs('/fsx_laion/alvin/Dataset/getty_human_posear0.5', exist_ok=True)
    # cnt1 = 0
    # cnt2 = 0
    cnt = 0
    ...
    for tar_file in all_tar_list:
        # try:
        # os.system(f"sudo rm -f {os.path.join('/fsx_laion/alvin/Dataset/getty_human_inpaint', tar_file.split('/')[-1])}")
        writer = wds.TarWriter(os.path.join("/fsx_laion/alvin/Dataset/getty_human_posear0.5", tar_file.split('/')[-1]))
        # writer = wds.TarWriter(os.path.join("/fsx_laion/alvin/Dataset/getty_human_posear0.5", '{:06d}.tar'.format(cnt1)))
        ...
        for i, sample in enumerate(dataset):
            ...
            if (face_kp.shape[0] == 1) and (body_kp.shape[0] == 1) and (body_kpconf > 0.5).all() and (body_kp[0, :15, 0] > 0).all() \
                ...
                if (x_max - x_min) * (y_max - y_min) / area > 0.5:
