...
def log_validation(vae, text_encoder, tokenizer, unet, args, accelerator, weight_dtype, global_step, batch=None, depth_embedder=None, normal_embedder=None, canny_embedder=None, body_embedder=None, face_embedder=None, hand_embedder=None, whole_embedder=None):
    ...
    # else:
        ...
        if "whole" in args.cond_type:
            # structural_cond.append(batch["whole"])
            whole_denormalize = (batch["whole"] + 1) / 2.0
            whole_numpy = whole_denormalize.cpu().permute(0, 2, 3, 1).float().numpy()[0]
            whole_numpy = (whole_numpy * 255).round().astype("uint8")
            whole_pil = Image.fromarray(whole_numpy)
            whole_pil.save(os.path.join(save_path, f"whole.png"))
...
def parse_args():
    ...
    parser.add_argument("--cond_num", type=int)
...
def main():
    ...
    from diffusers.models.unet_2d_condition_multi_branch import UNet2DConditionModel
    unet = UNet2DConditionModel.from_pretrained_2d(args.pretrained_model_name_or_path, subfolder="unet", size_cond=args.size_cond, cond_num=args.cond_num, copy_last_n_block=args.copy_last_n_block)
    ...
    if "face" in args.cond_type: num_cond += 1
    if "hand" in args.cond_type: num_cond += 1
    if "whole" in args.cond_type: num_cond += 1
    ...
    with torch.no_grad():
        ...
        elif args.copy_weight_same:
            unet.conv_in.weight[:, :4] = weights
            for i in range(num_cond):
                unet.conv_in.weight[:, 4 * (i + 1) : 4 * (i + 2)] = weights
        else:
            unet.conv_in.weight[:, :4] = weights
    ...
    if args.use_ema:
        from diffusers.models.unet_2d_condition_multi_branch import UNet2DConditionModel
        ema_unet = UNet2DConditionModel.from_pretrained_2d(args.pretrained_model_name_or_path, subfolder="unet", size_cond=args.size_cond, cond_num=args.cond_num, copy_last_n_block=args.copy_last_n_block)
        ...
        with torch.no_grad():
            if args.copy_weight:
                ...
                for i in range(num_cond):
                    ema_unet.conv_in.weight[:, 4 * (i + 1) : 4 * (i + 2)] = weights
            else:
                ema_unet.conv_in.weight[:, :4] = weights
    ...
    depth_embedder = normal_embedder = canny_embedder = body_embedder = face_embedder = hand_embedder = whole_embedder = None
    ...
    if args.cond_reshape == "learn_conv":
        ...
        if "whole" in args.cond_type:
            whole_embedder = Embedder(conditioning_embedding_channels=args.embedder_channel, conditioning_channels=3).to(accelerator.device, dtype=weight_dtype)
            params_to_optimize.extend(list(whole_embedder.parameters()))
    ...
    if args.resume_from_checkpoint:
        ...
        if path is None:
            ...
            if args.cond_reshape == "learn_conv":
                ...
                if "whole" in args.cond_type:
                    whole_state_dict = torch.load(os.path.join(args.output_dir, path, "whole_embedder.pth"))
                    new_state_dict = OrderedDict()
                    for k, v in whole_state_dict.items():
                        name = k[7:] if k[:6] == 'module' else k 
                        new_state_dict[name] = v
                    whole_embedder.load_state_dict(new_state_dict)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            images, text_input_ids, text_raw, blip, blip_raw, body, face, hand, normal, depth, canny, descriptions, body_ori, face_ori, hand_ori, normal_ori, depth_ori, canny_ori, whole, whole_ori = batch
            ...
            if "normal" in args.cond_type:
                ...
                batch["normal"] = normal
                if not args.pred_null:
                    batch["normal_ori"] = normal_ori.to(unet.device)
            if "depth" in args.cond_type:
                depth = depth.to(unet.device)
                batch["depth"] = depth
                if not args.pred_null:
                    batch["depth_ori"] = depth_ori.to(unet.device)
            if "canny" in args.cond_type:
                canny = canny.to(unet.device)
                batch["canny"] = canny
                if not args.pred_null:
                    batch["canny_ori"] = canny_ori.to(unet.device)
            if "body" in args.cond_type:
                body = body.to(unet.device)
                batch["body"] = body
                if not args.pred_null:
                    batch["body_ori"] = body_ori.to(unet.device)
            if "face" in args.cond_type:
                face = face.to(unet.device)
                batch["face"] = face
                if not args.pred_null:
                    batch["face_ori"] = face_ori.to(unet.device)
            if "hand" in args.cond_type:
                hand = hand.to(unet.device)
                batch["hand"] = hand
                if not args.pred_null:
                    batch["hand_ori"] = hand_ori.to(unet.device)
            if "whole" in args.cond_type:
                whole = whole.to(unet.device)
                batch["whole"] = whole
                if not args.pred_null:
                    batch["whole_ori"] = whole_ori.to(unet.device)
            ...
            with accelerator.accumulate(unet):
                ...
                if "depth" in args.cond_type:
                    if args.cond_reshape == "vae":
                        ...
                        if not args.pred_null:
                            undropped_depth_latents = vae.encode(batch["depth_ori"].to(weight_dtype)).latent_dist.sample()
                            undropped_depth_latents = undropped_depth_latents * vae.config.scaling_factor
                    elif args.cond_reshape == "resize":
                        depth_latents = F.interpolate(batch["depth"], (h,w))
                        if not args.pred_null:
                            undropped_depth_latents = F.interpolate(batch["depth_ori"], (h,w))
                    elif args.cond_reshape == "learn_conv":
                        depth_latents = depth_embedder(batch["depth"])
                        if not args.pred_null:
                            undropped_depth_latents = depth_embedder(batch["depth_ori"])
                    else:
                ...
                if "normal" in args.cond_type:
                    if args.cond_reshape == "vae":
                        ...
                        if not args.pred_null:
                            undropped_normal_latents = vae.encode(batch["normal_ori"].to(weight_dtype)).latent_dist.sample()
                            undropped_normal_latents = undropped_normal_latents * vae.config.scaling_factor
                    elif args.cond_reshape == "resize":
                        normal_latents = F.interpolate(batch["normal"], (h,w))
                        if not args.pred_null:
                            undropped_normal_latents = F.interpolate(batch["normal_ori"], (h,w))
                    elif args.cond_reshape == "learn_conv":
                        normal_latents = normal_embedder(batch["normal"])
                        if not args.pred_null:
                            undropped_normal_latents = normal_embedder(batch["normal_ori"])
                    else:
                        assert False, "unknown condition reshape type"
                ...
                if "canny" in args.cond_type:
                    if args.cond_reshape == "vae":
                        ...
                        if not args.pred_null:
                            undropped_canny_latents = vae.encode(batch["canny_ori"].to(weight_dtype)).latent_dist.sample()
                            undropped_canny_latents = undropped_canny_latents * vae.config.scaling_factor
                    elif args.cond_reshape == "resize":
                        canny_latents = F.interpolate(batch["canny"], (h,w))
                        if not args.pred_null:
                            undropped_canny_latents = F.interpolate(batch["canny_ori"], (h,w))
                    elif args.cond_reshape == "learn_conv":
                        canny_latents = canny_embedder(batch["canny"])
                        if not args.pred_null:
                            undropped_canny_latents = canny_embedder(batch["canny_ori"])
                    else:
                        assert False, "unknown condition reshape type"
                ...
                if "body" in args.cond_type:
                    if args.cond_reshape == "vae":
                        ...
                        if not args.pred_null:
                            undropped_body_latents = vae.encode(batch["body_ori"].to(weight_dtype)).latent_dist.sample()
                            undropped_body_latents = undropped_body_latents * vae.config.scaling_factor
                    elif args.cond_reshape == "resize":
                        body_latents = F.interpolate(batch["body"], (h,w))
                        if not args.pred_null:
                            undropped_body_latents = F.interpolate(batch["body_ori"], (h,w))
                    elif args.cond_reshape == "learn_conv":
                        body_latents = body_embedder(batch["body"])
                        if not args.pred_null:
                            undropped_body_latents = body_embedder(batch["body_ori"])
                    else:
                        assert False, "unknown condition reshape type"
                ...
                if "face" in args.cond_type:
                    if args.cond_reshape == "vae":
                        ...
                        if not args.pred_null:
                            undropped_face_latents = vae.encode(batch["face_ori"].to(weight_dtype)).latent_dist.sample()
                            undropped_face_latents = undropped_face_latents * vae.config.scaling_factor
                    elif args.cond_reshape == "resize":
                        face_latents = F.interpolate(batch["face"], (h,w))
                        if not args.pred_null:
                            undropped_face_latents = F.interpolate(batch["face_ori"], (h,w))
                    elif args.cond_reshape == "learn_conv":
                        face_latents = face_embedder(batch["face"])
                        if not args.pred_null:
                            undropped_face_latents = face_embedder(batch["face_ori"])
                    else:
                        assert False, "unknown condition reshape type"
                ...
                if "hand" in args.cond_type:
                    if args.cond_reshape == "vae":
                        ...
                        if not args.pred_null:
                            undropped_hand_latents = vae.encode(batch["hand_ori"].to(weight_dtype)).latent_dist.sample()
                            undropped_hand_latents = undropped_hand_latents * vae.config.scaling_factor
                    elif args.cond_reshape == "resize":
                        hand_latents = F.interpolate(batch["hand"], (h,w))
                        if not args.pred_null:
                            undropped_hand_latents = F.interpolate(batch["hand_ori"], (h,w))
                    elif args.cond_reshape == "learn_conv":
                        hand_latents = hand_embedder(batch["hand"])
                        if not args.pred_null:
                            undropped_hand_latents = hand_embedder(batch["hand_ori"])
                    else:
                        assert False, "unknown condition reshape type"
                        
                if "whole" in args.cond_type:
                    if args.cond_reshape == "vae":
                        whole_latents = vae.encode(batch["whole"].to(weight_dtype)).latent_dist.sample()
                        whole_latents = whole_latents * vae.config.scaling_factor
                        if not args.pred_null:
                            undropped_whole_latents = vae.encode(batch["whole_ori"].to(weight_dtype)).latent_dist.sample()
                            undropped_whole_latents = undropped_whole_latents * vae.config.scaling_factor
                    elif args.cond_reshape == "resize":
                        whole_latents = F.interpolate(batch["whole"], (h,w))
                        if not args.pred_null:
                            undropped_whole_latents = F.interpolate(batch["whole_ori"], (h,w))
                    elif args.cond_reshape == "learn_conv":
                        whole_latents = whole_embedder(batch["whole"])
                        if not args.pred_null:
                            undropped_whole_latents = whole_embedder(batch["whole_ori"])
                    else:
                        assert False, "unknown condition reshape type"
                ...
                if "whole" in args.cond_type:
                    ori_whole_latents = whole_latents
                    if "whole" in args.noisy_cond:
                        noise_whole = torch.randn_like(whole_latents)
                        whole_latents = noise_scheduler.add_noise(whole_latents, noise_whole, timesteps)       
                ...
                if args.cond_inject == "concat":
                    ...
                    noisy_latents = torch.cat([noisy_latents, whole_latents], dim=1) if "whole" in args.cond_type else noisy_latents
                elif args.cond_inject == "sum":
                    if len(args.cond_type) == 0:
                        ...
                        sum_latents = sum_latents + whole_latents if "whole" in args.cond_type else sum_latents
                ...
                if noise_scheduler.config.prediction_type == "epsilon":
                    target = noise
                    if "depth" in args.cond_type and "depth" in args.pred_cond:
                        if "depth" in args.noisy_cond:
                            target_depth = noise_depth
                        else:
                            if args.pred_null:
                                target_depth = ori_depth_latents
                            else:
                                target_depth = undropped_depth_latents
                    if "normal" in args.cond_type and "normal" in args.pred_cond:
                        if "normal" in args.noisy_cond:
                            target_normal = noise_normal
                        else:
                            if args.pred_null:
                                target_normal = ori_normal_latents
                            else:
                                target_normal = undropped_normal_latents
                    if "canny" in args.cond_type and "canny" in args.pred_cond:
                        if "canny" in args.noisy_cond:
                            target_canny = noise_canny
                        else:
                            if args.pred_null:
                                target_canny = ori_canny_latents
                            else:
                                target_canny = undropped_canny_latents
                    if "body" in args.cond_type and "body" in args.pred_cond:
                        if "body" in args.noisy_cond:
                            target_body = noise_body
                        else:
                            if args.pred_null:
                                target_body = ori_body_latents
                            else:
                                target_body = undropped_body_latents
                    if "face" in args.cond_type and "face" in args.pred_cond:
                        if "face" in args.noisy_cond:
                            target_face = noise_face
                        else:
                            if args.pred_null:
                                target_face = ori_face_latents
                            else:
                                target_face = undropped_face_latents
                    if "hand" in args.cond_type and "hand" in args.pred_cond:
                        if "hand" in args.noisy_cond:
                            target_hand = noise_hand
                        else:
                            if args.pred_null:
                                target_hand = ori_hand_latents
                            else:
                                target_hand = undropped_hand_latents
                    if "whole" in args.cond_type and "whole" in args.pred_cond:
                        if "whole" in args.noisy_cond:
                            target_whole = noise_whole
                        else:
                            if args.pred_null:
                                target_whole = ori_whole_latents
                            else:
                                target_whole = undropped_whole_latents
                    ...
                    if "depth" in args.cond_type and "depth" in args.pred_cond:
                        if "depth" in args.noisy_cond:
                            target_depth = noise_scheduler.get_velocity(ori_depth_latents, noise_depth, timesteps)
                        else:
                            if args.pred_null:
                                target_depth = ori_depth_latents
                            else:
                    ...
                    if "normal" in args.cond_type and "normal" in args.pred_cond:
                        if "normal" in args.noisy_cond:
                            target_normal = noise_scheduler.get_velocity(ori_normal_latents, noise_normal, timesteps)
                        else:
                            if args.pred_null:
                                target_normal = ori_normal_latents
                            else:
                    ...
                    if "canny" in args.cond_type and "canny" in args.pred_cond:
                        if "canny" in args.noisy_cond:
                            target_canny = noise_scheduler.get_velocity(ori_canny_latents, noise_canny, timesteps)
                        else:
                            if args.pred_null:
                                target_canny = ori_canny_latents
                            else:
                    ...
                    if "body" in args.cond_type and "body" in args.pred_cond:
                        if "body" in args.noisy_cond:
                            target_body = noise_scheduler.get_velocity(ori_body_latents, noise_body, timesteps)
                        else:
                            if args.pred_null:
                                target_body = ori_body_latents
                            else:
                    ...
                    if "face" in args.cond_type and "face" in args.pred_cond:
                        if "face" in args.noisy_cond:
                            target_face = noise_scheduler.get_velocity(ori_face_latents, noise_face, timesteps)
                        else:
                            if args.pred_null:
                                target_face = ori_face_latents
                            else:
                    ...
                    if "hand" in args.cond_type and "hand" in args.pred_cond:
                        if "hand" in args.noisy_cond:
                            target_hand = noise_scheduler.get_velocity(ori_hand_latents, noise_hand, timesteps)
                        else:
                            if args.pred_null:
                                target_hand = ori_hand_latents
                            else:
                    ...
                    if "whole" in args.cond_type and "whole" in args.pred_cond:
                        if "whole" in args.noisy_cond:
                            target_whole = noise_scheduler.get_velocity(ori_whole_latents, noise_whole, timesteps)
                        else:
                            if args.pred_null:
                                target_whole = ori_whole_latents
                            else:
                ...
                if "hand" in args.cond_type and "hand" in args.pred_cond:
                    ...
                    loss += hand_loss
                if "whole" in args.cond_type and "whole" in args.pred_cond:
                    whole_loss = F.mse_loss(whole_pred.float(), target_whole.float(), reduction="mean") * args.whole_weight
                    loss += whole_loss
                    ...
                    if accelerator.is_main_process:
                        ...
                        if not os.path.exists(save_path):
                            ...
                            if args.cond_reshape == "learn_conv":
                                ...
                                if "whole" in args.cond_type:
                                    torch.save(whole_embedder.state_dict(), os.path.join(save_path, "whole_embedder.pth"))
            ...
            if "whole" in args.cond_type and "whole" in args.pred_cond:
                logs["whole"] = whole_loss.detach().item()
            progress_bar.set_postfix(**logs)

            if accelerator.is_main_process:
                # if args.validation_prompts is not None and epoch % args.validation_epochs == 0:
                if args.validation_prompts is not None and global_step % args.validation_steps == 0 and args.timestep_start == 0 and args.timestep_end == 1000:
                    if args.use_ema:
