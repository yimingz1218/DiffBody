...
def log_validation(vae, text_encoder, tokenizer, unet, args, accelerator, weight_dtype, global_step, test_dataloader):
...
def parse_args():
    ...
    parser.add_argument("--lw_rgb", type=float, default=1.0)
    parser.add_argument("--lw_depth", type=float, default=1.0)
    parser.add_argument("--lw_normal", type=float, default=1.0)
...
def main():
    ...
    # writer = SummaryWriter(args.output_dir, args.logging_dir)
    ...
    data_args.train_data = data_args.train_data[::-1]
    ...
    if accelerator.is_main_process and args.timestep_start == 0 and args.timestep_end == 1000 and args.prediction_type == "epsilon":
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            for i_sample, description in enumerate(descriptions):
                instance_unet_added_conditions = compute_embeddings(
                    ...
                    description["aes"], description["watermark"],
                )
            ...
            midas_depth = midas_depth.to(unet.device)
            batch["midas_depth"] = midas_depth
            batch["midas_depth_ori"] = midas_depth_ori.to(unet.device)
            ...
            with accelerator.accumulate(unet):
                ...
                if args.normalize_dist:
                    midas_depth_latents = (midas_depth_latents - args.depth_mean) / args.depth_std * args.rgb_std + args.rgb_mean
                ...
                if args.normalize_dist:
                    normal_latents = (normal_latents - args.normal_mean) / args.normal_std * args.rgb_std + args.rgb_mean
                ...
                if args.normalize_dist:
                    whole_latents = (whole_latents - args.whole_mean) / args.whole_std * args.rgb_std + args.rgb_mean
                ...
                noisy_latents_with_cond = torch.cat([noisy_latents, whole_latents], dim=1)
                    
                noisy_latents_list = [torch.cat([midas_depth_latents, whole_latents], dim=1), torch.cat([normal_latents, whole_latents], dim=1)]
                ...
                if noise_scheduler.config.prediction_type == "epsilon":
                    ...
                    target_midas_depth = noise_scheduler.get_velocity(ori_midas_depth_latents, noise_midas_depth, timesteps)
                ...
                target_all = torch.cat([target, target_midas_depth, target_normal], dim=1)
                
                # if accelerator.is_local_main_process:
                #     time3 = time.time()
                model_pred = unet(noisy_latents_with_cond, noisy_latents_list, timesteps, encoder_hidden_states, added_cond_kwargs=batch["unet_added_conditions"]).sample
                # if accelerator.is_local_main_process:
                #     time4 = time.time()
                # writer.add_graph(unet, input_to_model=(noisy_latents_with_cond, noisy_latents_list, timesteps, encoder_hidden_states), use_strict_trace=False)
                # writer.close()

                # rgb_loss = F.mse_loss(model_pred[:, :4].float(), target.float(), reduction="mean") * args.lw_rgb
                # midas_depth_loss = F.mse_loss(model_pred[:, 4:8].float(), target_midas_depth.float(), reduction="mean") * args.lw_depth
                # normal_loss = F.mse_loss(model_pred[:, 8:12].float(), target_normal.float(), reduction="mean") * args.lw_normal
                    
                # loss = rgb_loss + midas_depth_loss + normal_loss
                loss = F.mse_loss(model_pred.float(), target_all.float(), reduction="mean")
            ...
            # logs = {"step_loss": loss.detach().item(), "lr": lr_scheduler.get_last_lr()[0], "rgb_loss": rgb_loss.detach().item(), "midas_loss": midas_depth_loss.detach().item(), "normal_loss": normal_loss.detach().item()}
            logs = {"step_loss": loss.detach().item(), "lr": lr_scheduler.get_last_lr()[0]}
            progress_bar.set_postfix(**logs)

            if accelerator.is_main_process:
                # if args.validation_prompts is not None and epoch % args.validation_epochs == 0:
                if global_step % args.validation_steps == 0 and args.timestep_start == 0 and args.timestep_end == 1000:
                    if args.use_ema:
