...
from pipelines.pipeline_stable_diffusion_spade_timemoe5 import StableDiffusionPipeline
...
def parse_args():
    ...
    parser.add_argument("--inference_res", type=int)
...
def main():
    ...
    unet_0_200 = UNet2DConditionModel.from_pretrained("/fsx_laion/alvin/sd4human/rc-sdv21-base-v-glc-composer-time-0-200-512/checkpoint-34000", subfolder="unet_ema")
    unet_0_200.requires_grad_(False)

    unet_200_400 = UNet2DConditionModel.from_pretrained("/fsx_laion/alvin/sd4human/rc-sdv21-base-v-glc-composer-time-200-400-512/checkpoint-34000", subfolder="unet_ema")
    unet_200_400.requires_grad_(False)
    
    unet_400_600 = UNet2DConditionModel.from_pretrained("/fsx_laion/alvin/sd4human/rc-sdv21-base-v-glc-composer-time-400-600-512/checkpoint-47000", subfolder="unet_ema")
    unet_400_600.requires_grad_(False)
    
    unet_600_800 = UNet2DConditionModel.from_pretrained("/fsx_laion/alvin/sd4human/rc-sdv21-base-v-glc-composer-time-600-800-512/checkpoint-32000", subfolder="unet_ema")
    unet_600_800.requires_grad_(False)
    
    unet_800_1000 = UNet2DConditionModel.from_pretrained("/fsx_laion/alvin/sd4human/rc-sdv21-base-v-glc-composer-time-800-1000-512/checkpoint-32000", subfolder="unet_ema")
    unet_800_1000.requires_grad_(False)
    ...
    if args.enable_xformers_memory_efficient_attention:
        if is_xformers_available():
            ...
            unet_0_200.enable_xformers_memory_efficient_attention()
            unet_200_400.enable_xformers_memory_efficient_attention()
            unet_400_600.enable_xformers_memory_efficient_attention()
            unet_600_800.enable_xformers_memory_efficient_attention()
            unet_800_1000.enable_xformers_memory_efficient_attention()
        else:
    ...
    params_to_optimize = list(unet_0_200.parameters()) 
    ...
    # unet_0_200, optimizer, train_dataloader, lr_scheduler = accelerator.prepare(
    #     unet_0_200, optimizer, train_dataloader, lr_scheduler
    # )
    ...
    # if args.use_ema:
    #     # Store the unet_0_200 parameters temporarily and load the EMA parameters to perform inference.
    #     ema_unet.store(unet_0_200.parameters())
    #     ema_unet.copy_to(unet_0_200.parameters())
    ...
    pipeline = StableDiffusionPipeline.from_pretrained(
        ...
        tokenizer=tokenizer,
        unet_0_200=unet_0_200,
        unet_200_400=unet_200_400,
        unet_400_600=unet_400_600,
        unet_600_800=unet_600_800,
        unet_800_1000=unet_800_1000,
        safety_checker=None,
    ...
    for step, batch in enumerate(test_dataloader):
        ...
        images = images.to(unet_0_200.device)
        ...
        if "normal" in args.cond_type:
            normal = normal.to(unet_0_200.device)
            batch["normal"] = normal
        if "depth" in args.cond_type:
            depth = depth.to(unet_0_200.device)
            batch["depth"] = depth
        if "canny" in args.cond_type:
            canny = canny.to(unet_0_200.device)
            batch["canny"] = canny
        if "body" in args.cond_type:
            body = body.to(unet_0_200.device)
            batch["body"] = body
        if "face" in args.cond_type:
            face = face.to(unet_0_200.device)
            batch["face"] = face
        if "hand" in args.cond_type:
            hand = hand.to(unet_0_200.device)
            batch["hand"] = hand
        ...
        for i_batch in range(batch_size):
            ...
            if "depth" in args.cond_type:
                ...
                if "depth" in args.cond_type_test:
                    ...
                    batch["depth"] = (torch.ones_like(batch["depth"][0].unsqueeze(0)) * (-1)).to(unet_0_200.device)
            if "normal" in args.cond_type:
                ...
                if "normal" in args.cond_type_test:
                    ...
                    batch["normal"] = (torch.ones_like(batch["normal"][0].unsqueeze(0)) * (-1)).to(unet_0_200.device)
            if "canny" in args.cond_type:
                ...
                if "canny" in args.cond_type_test:
                    ...
                    batch["canny"] = (torch.ones_like(batch["canny"][0].unsqueeze(0)) * (-1)).to(unet_0_200.device)
            if "body" in args.cond_type:
                ...
                if "body" in args.cond_type_test:
                    ...
                    batch["body"] = (torch.ones_like(batch["body"][0].unsqueeze(0)) * (-1)).to(unet_0_200.device)
            if "face" in args.cond_type:
                ...
                if "face" in args.cond_type_test:
                    ...
                    batch["face"] = (torch.ones_like(batch["face"][0].unsqueeze(0)) * (-1)).to(unet_0_200.device)
            if "hand" in args.cond_type:
                ...
                if "hand" in args.cond_type_test:
                    ...
                    batch["hand"] = (torch.ones_like(batch["hand"][0].unsqueeze(0)) * (-1)).to(unet_0_200.device)
            ...
            with torch.autocast("cuda"):
                output = pipeline(
                    ...
                    original_size=(args.inference_res, args.inference_res),
                ...
                if "depth" in args.noisy_cond:
                    ...
                    depth_image.save(os.path.join(save_path, f"{img_id}-depth.png"))
                ...
                if "canny" in args.noisy_cond:
                    ...
                    canny_image.save(os.path.join(save_path, f"{img_id}-canny.png"))
                if "body" in args.noisy_cond:
                    body_image = output.body_image[0]
                    body_image.save(os.path.join(save_path, f"{img_id}-body.png"))
                if "face" in args.noisy_cond:
                    face_image = output.face_image[0]
                    face_image.save(os.path.join(save_path, f"{img_id}-face.png"))
                if "hand" in args.noisy_cond:
                    hand_image = output.hand_image[0]
                    hand_image.save(os.path.join(save_path, f"{img_id}-hand.png"))
    ...
    #         file.write(item + '\n')
        ...
        # if args.use_ema:
        #     # Switch back to the original unet_0_200 parameters.
        #     ema_unet.restore(unet_0_200.parameters())
