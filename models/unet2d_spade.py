...
try:
    ...
    from models.unet2d_spade_blocks import (
        CrossAttnDownBlock2D,
        CrossAttnUpBlock2D,
        DownBlock2D,
        UNetMidBlock2DCrossAttn,
...
class UNet2DConditionModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    ...
    def forward(
        ...
        for i, upsample_block in enumerate(self.up_blocks):
            ...
            if hasattr(upsample_block, "has_cross_attention") and upsample_block.has_cross_attention:
                ...
                sample = upsample_block(
                    hidden_states=sample, seg=seg, temb=emb, res_hidden_states_tuple=res_samples, upsample_size=upsample_size
                )
    ...
    @classmethod
    def from_pretrained_2d(cls, pretrained_model_path, weight=None, subfolder=None, label_channels=3):
        ...
        config["label_channels"] = label_channels
...
if __name__ == '__main__':
    ...
    pretrained_model_path = "/fsx_laion/alvin/pretrain/stable-diffusion-v1-5"
    unet = UNet2DConditionModel.from_pretrained_2d(pretrained_model_path, subfolder="unet", label_channels=17).to(device)

    noisy_latents = torch.randn((8, 4, 64, 64)).to(device)
    seg = torch.randn((8, 17, 512, 512)).to(device)
    bsz = noisy_latents.shape[0]
    timesteps = torch.randint(0, 1000, (bsz,)).to(device)
    timesteps = timesteps.long()
    encoder_hidden_states = torch.randn((bsz, 50, 768)).to(device)

    model_pred = unet(noisy_latents, seg, timesteps, encoder_hidden_states).sample
